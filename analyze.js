export default async function handler(req, res) {
  ...
  const prompt = `첨부한 이미지는 학원 "개별지도 시간표" 공지 표입니다. 표의 왼쪽에는 학생 이름, 과목(●색상점 + 과목명 + 숫자꼼마 형태)이 있고, 오른쪽에는 월~일 요일별 칸에 시간대를 뜻하는 알파벳 코드(A~H 등)가 적혀 있습니다(빈 칸도 많습니다).
...
반드시 아래 JSON 형식으로만 답해줘.
{
  ...
  "subjects": [ { "name": "과목명(예: 언매 3꼼마)", "codes": ["E","","","","F","E",""] } ],
